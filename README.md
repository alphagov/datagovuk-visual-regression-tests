...
- Backstop has the option to ignore portions of html that we don't want to include in tests eg: fields that relate to times that will be constantly changing. Unfortunately this is very difficult to police in our ckan instaces as we use a mix of markup from our own extension and default ckan admin panel markup, making it a near impossble job to apply selectors to all unwanted fields across the ckan admin panel.
